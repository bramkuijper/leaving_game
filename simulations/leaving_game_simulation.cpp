...
void init_population(int argc, char **argv)
{
    ...
    l1_init  = std::stod(argv[4]);
    l2_init  = std::stod(argv[5]);
    ...
    standard_individual.l[0] = l1_init;
    standard_individual.l[1] = l2_init;
...
{
    double meanl[2]{0.0,0.0};
    double ssl[2]{0.0,0.0};
    ...
    for (unsigned group_idx{0}; group_idx < n_groups; ++group_idx)
    {
        ...
        {
            for (unsigned element_idx{0}; element_idx < 2; ++element_idx)
            {
                l = population[group_idx].members[member_idx].l[element_idx];
    ...
    double varl[2]{ssl[0] / n - meanl[0] * meanl[0], 
        ssl[1] / n - meanl[1] * meanl[1]};
...
int main(int argc, char **argv)
{
    ...
    output_file << "generation;meanl1;meanl2;varl1;varl2;mean_stay1;mean_stay2;" << std::endl;
